...
function update_env_with_ngrok_urls() {
    ...
    # public/hot is written by Vite itself on (re)start; vite.config.js reads
    # NGROK_DELTA_VITE_URL and the .env edits above trigger Vite's dev-server
    # restart (it watches .env), so it picks up the new tunnel URL on its own.
